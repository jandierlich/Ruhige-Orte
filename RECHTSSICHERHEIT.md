...
## Nachtrag 13 (Claude, 16.08.2026) – Professionelleres Farb-/Optik-Redesign, erneute Rechtsprüfung

Auf Jans Wunsch komplettes optisches Redesign in allen fünf Seiten (`index.html`, `app.html`, `anleitung.html`, `impressum.html`, `datenschutz.html`):

- **Neue Farbpalette:** Von hellem, verspieltem Himmelblau/Pastell auf ein ruhigeres, hochwertigeres Schema aus gedecktem Waldgrün/Tannengrün (`--accent:#2C5049`, `--primary:#3C6E62`) und warmem Papier-/Sandton statt Blau-Verlauf als Hintergrund umgestellt. Wirkt zurückhaltender und passender zu „ruhigen Orten" als das vorherige knallige Himmelblau. Dark Mode entsprechend nachgezogen.
- **Verspielte Elemente entfernt:** Die animierten, schwebenden Hintergrund-„Orbs" (blaue verschwommene Kreise) wurden entfernt – lenkten optisch ab und wirkten eher nach Spiele-App als nach seriösem Werkzeug. Ruhiger, statischer Verlaufshintergrund bleibt.
- **Buttons/Pills entschärft:** Die bisherigen glänzenden 3D-„Bonbon"-Schatten (mehrfache Inset-Highlights, dicke Kanten) durch flache, dezente Schatten im Stil moderner professioneller Software ersetzt. Eckenradien leicht reduziert (Karten/Buttons von sehr rund auf gemäßigt rund), Schriftgewicht der Überschriften von 800 auf 700 reduziert für ruhigere Typografie.
- **Kartenrahmen** von dickem 8px-Weiß-Rahmen auf schlanken 3px-Rahmen reduziert, Kartenkacheln-Sättigung leicht gedämpft (weniger grell, natürlicher).
- **Alle Marker-, Button- und Inline-Farben in `app.html` (Nutzerstandort-Marker, Umkreis, OSM-Marker, Live-Suche-Button, Foto-Löschen-Button, Druckansicht) auf die neue Palette angeglichen** – keine funktionale Änderung, nur Farbwerte.
- **`manifest.json`** (`theme_color`/`background_color`) und alle `<meta name="theme-color">`-Tags auf die neue Akzentfarbe angepasst, damit iOS-Statusleiste/Homescreen zur neuen Optik passen.
- **Cache-Version in `sw.js` auf v23 erhöht**, damit bereits installierte Nutzer die neue Optik zuverlässig erhalten.

**Erneute lückenlose Rechtsprüfung vor dieser Änderung:** Impressum (§5 TMG, §18 MStV) korrekt und vollständig, Datenschutzerklärung nennt alle externen Dienste (OpenStreetMap-Kacheln, Leaflet/unpkg, Nominatim, Overpass inkl. aller Spiegelserver, Open-Meteo, GitHub Pages) mit Opt-in-Hinweisen wo nötig, MIT-Lizenz korrekt, keine Google Fonts/kein Tracking/keine Cookies. Das Redesign betrifft ausschließlich CSS-Werte, Inline-Style-Farben und Markup-Entfernung der Deko-Orbs – keine neuen externen Dienste, keine neuen Datenflüsse, keine funktionalen Änderungen. Rechtlich damit unverändert unbedenklich.
